...
export default class Halfofflist extends Component {
    constructor(props) {
        ...
        //console.log(_.filter(global.All, e => e.available== 1 && e.promotion == 1));

        //all wine halfoff available
        db.transaction(tx => {
            tx.executeSql('SELECT * FROM ipad_wines WHERE available= ? AND promotion = ?', [1,1], (tx, results) => {
              var temp = [];
