    ...
    onPressPlus = (id, type) => {
     ...
     onPressMoinPlus = (id, type="byglass") => {
         ...
         if (this.getSelected(id,type) != undefined) {
            ...
            <ImageBackground source={require('../img/plusr.png')} style={{ height: hp('1.8%'),width:wp('2.5%')}}>
            </ImageBackground>
        </TouchableOpacity>
    }
    else return <TouchableOpacity  onPress={this.onPressPlus.bind(this, id, type)}>
        <ImageBackground source={require('../img/plusr.png')} style={{ height: hp('1.8%'),width:wp('2.5%'), marginLeft: 45}}>
        </ImageBackground>
    </TouchableOpacity>;
        }
